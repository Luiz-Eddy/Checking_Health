  ...
  if (isset($_POST['emailRecupera']) | isset($_POST['codeRecupera'])) {
    ...
    if(mysqli_num_rows($verificar) == 1 && mysqli_num_rows($verificar1) == 1){
      ...
      if (mysqli_num_rows($verificar2) == 1) {
        $att_processo = "UPDATE recovery_codes SET process = 1 WHERE id_usuario = '$id_user' AND code = '$code';";
        $exec_att = mysqli_query($conn,$att_processo);

        header("Location: ../nova_senha.php?mail=$email&code=$code");
      }else{
        $_SESSION['msg'] = "<div class='toast-cad-false'>Informações incorretas 1</div>";
        header('Location: ../login.php');
        exit();
      }
    }else{
      $_SESSION['msg'] = "<div class='toast-cad-false'>Informações incorretas 2</div>";
      echo $email, $code;
      // header('Location: ../login.php');
      // exit();
    }
  }else{
    $_SESSION['msg'] = "<div class='toast-cad-false'>Informações incorretas 3</div>";
    header('Location: ../login.php');
    exit();
  }
?>